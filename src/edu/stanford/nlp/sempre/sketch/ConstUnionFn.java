...
public class ConstUnionFn extends SemanticFn {
    ...
    public ConstUnionFn(int position0, int position1) {
    ...
    public DerivationStream call(Example ex, final Callable c) {
        return new SingleDerivationStream() {
            ...
            public Derivation createDerivation() {
                ...
                NameValue formula;
                if (targetCat.equals("$CONST_SET")) {
                    // execute
                    String[] args = (arg0 + " " + arg1).split(" ");
                    formula = new NameValue("or(" + args[position0] + "," + args[position1] + ")");
                } else {
