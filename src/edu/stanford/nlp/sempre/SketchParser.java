 ...
 * @author Percy Liang
 */
public class SketchParser extends Parser {
    ...
    public ParserState newParserState(Params params, Example ex, boolean computeExpectedCounts) {
        SketchParserState coarseState = null;
        if (Parser.opts.coarsePrune) {
            LogInfo.begin_track("Parser.coarsePrune");
            coarseState = new SketchParserState(this, params, ex, computeExpectedCounts, SketchParserState.Mode.bool, null);
            coarseState.infer();
            coarseState.keepTopDownReachable();
            LogInfo.end_track();
        }
        return new SketchParserState(this, params, ex, computeExpectedCounts, SketchParserState.Mode.full, coarseState);
    }
}
 ...
 * @author Roy Frostig
 */
class SketchParserState extends ChartParserState {
    ...
    private final SketchParser parser;
    private final SketchParserState coarseState;  // Used to prune

    public SketchParserState(SketchParser parser, Params params, Example ex, boolean computeExpectedCounts,
                           Mode mode, SketchParserState coarseState) {
    ...
    protected List<Derivation> removeRedundancy(List<Derivation> derivations) {
        Comparator<Derivation> derivComparator = Derivation.derivScoreComparator;
        Map<String, Derivation> bestDeriv = new HashMap<>();
//        List<Derivation> newDerivations = Collections.emptyList();

        for (Derivation deriv : derivations) {
            String formula = Formulas.getString(deriv.formula);
            Derivation existedDeriv = bestDeriv.get(formula);
            if (existedDeriv == null) {
                bestDeriv.put(formula, deriv);
            } else {
                // its wierd, but it means existedDeriv.score < deriv.score
                if (derivComparator.compare(existedDeriv, deriv) > 0) {
                    bestDeriv.put(formula, deriv);
                }
            }
        }
        return new ArrayList<>(bestDeriv.values());
    }
    ...
    protected void pruneCell(Set<String> cellsPruned, String cat, int start, int end, List<Derivation> derivations) {
        ...
        // do inplace operation
        List<Derivation> newDerivations = removeRedundancy(derivations);
        chart[start][end].put(cat, newDerivations);
        pruneCell(cell, newDerivations);
    }
        ...
        for (int j = i + 1; j <= end; j++) {
            for (Map.Entry<String, List<Derivation>> entry : chart[i][j].entrySet()) {
                ...
                for (Derivation arg : entry.getValue()) {
                    ...
                    if (numNew.value >= SketchParser.opts.maxNewTreesPerSpan) return;
                }
            }
        }
    }
